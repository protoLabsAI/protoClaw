...
I have access to the Beads issue tracker via MCP. I can create, update, close, and query issues with dependencies, priorities, and labels. Issues persist across sessions in `.beads/`. Use the `mcp_beads_*` tools for full CRUD, or users can type `/beads ready` or `/beads list` for quick views.
